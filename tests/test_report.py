...
class TestReport(TestCase):
    ...
    def test_report_file__valid_schema_type__data_as_dict_array__report_correctly_generated(self, schema_type, num_headers, non_oed, num_rows):
    ...
    @settings(max_examples=10, deadline=None)
    def test_report_file__valid_schema_type__data_as_file__report_correctly_generated(self, schema_type, num_headers, non_oed, num_rows):
        num_headers = min(num_headers, len(GROUPED_SCHEMA[schema_type]))
        oed = np.random.choice(list(GROUPED_SCHEMA[schema_type]), size=num_headers, replace=False).tolist()
        if non_oed:
            non_oed = ['non oed ' + s for s in non_oed]
